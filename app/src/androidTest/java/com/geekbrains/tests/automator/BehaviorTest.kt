...
class BehaviorTest {
    ...
    fun test_SearchIsPositive() {
        ...
        uiDevice.click(uiDevice.displayWidth-20, uiDevice.displayHeight-20)
    ...
    //Убеждаемся, что DetailsScreen открывается и получает верные данные
    @Test
    fun test_SearchIsPositiveAndOpenSearchScreen() {
        //Через uiDevice находим editText
        val editText = uiDevice.findObject(By.res(packageName, "searchEditText"))
        editText.click()
        //Устанавливаем значение
        ...
        uiDevice.waitForIdle(600)
        uiDevice.click(uiDevice.displayWidth-25, uiDevice.displayHeight-25)
        ...
        // проверяем что передано верное значение
        Assert.assertEquals(resultedText.text, "Number of results: 42")
    }
